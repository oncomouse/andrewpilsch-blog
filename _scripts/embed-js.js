...
const processInclude = (el, type, files) => {
  ...
  if (src.indexOf('googleapis.com') >= 0) return;
  if (src.indexOf('twitter.com') >= 0) return;
  if (!files.has(src)) {
...
argv._.forEach((file) => {
  ...
  if (!argv.inline) {
    ...
    if (seenCss) {
      const cssPath = files.getNames(/main\.css$/)[0].replace('main', 'packed');
    ...
    if (seenJs) {
      const jsPath = files.getNames(/main\.js$/)[0].replace('main', 'packed');
